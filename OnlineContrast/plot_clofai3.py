...
prec = load_pad_to6(clofai_prefix+'class_wise_precision.csv')  # (T, 6)
rec  = load_pad_to6(clofai_prefix+'class_wise_recall.csv')  # (T, 6)
# -------- 颜色方案（n 种颜色）--------
# n<=20 用 tab20，更多时退回到 'hsv'
#cmap = plt.cm.get_cmap('tab20', n) if n <= 20 else plt.cm.get_cmap('hsv', n)
#colors = [cmap(i) for i in range(n)]
colors = ['r','g','b','c','m','y','orange','purple','brown','pink','gray','olive']
# -------- 画布与子图（先只画第一个）--------
...
ax2.plot(x,acc[:,1],linestyle='dashdot', linewidth=3,
        color='b', alpha=0.95 )
ax2.set_title('Averaged Multi-Classes Accuracy (dashdot)')
ax2.set_ylabel('')
