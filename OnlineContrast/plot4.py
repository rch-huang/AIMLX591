...
def plotting_acc(filename,figname=None,idx_cc_1=0,idx_cc_2=9):
    ...
    data ={"acc_knn_training_set":acc_knn_training_set,
        ...
        "times_cls":times_cls}
    for k in range(10):
        data["acc_distinguish_"+str(k)] = {int(key):data_raw["acc_distinguish_"+str(k)][key] for key in data_raw["acc_distinguish_"+str(k)].keys()}

    colors = ['r', 'g', 'b', 'c', 'm', 'y', 'k', 'orange', 'purple', 'brown']  # 10 different colors
    linestyles = ['-', '--', '-.', ':', ' ', '', 'solid', 'dashed', 'dashdot', 'dotted']  # 2 line styles for key2 and key3

    num_of_acc_curves = len(data["acc_knn_training_set"][0])-1
    ...
    for i in range(num_of_acc_curves):
        ...
        for j in [idx_cc_1,idx_cc_2]:
            y = [data["acc_distinguish_"+str(j)][k][i] for k in x_shared]
            ax2.plot(x_shared, y, color=colors[j % len(colors)], label=str(j+1))
    ...
    if figname != None:
        fig.savefig(figname+"_"+str(idx_cc_1)+str(idx_cc_2)+".jpg", format='jpg', dpi=300, bbox_inches='tight')
    else:
...
if __name__ == '__main__':
    ...
    for c in cc:
        plotting_acc(filename,None,c[0],c[1])